...
class TestSubscribes:
    def test_add_delete_subscribe_user(
        self,
        auth_client,
        auth_client_another,
        another_user,
        ...
        response = auth_client.post(subscribe_user_url)
        ...
        response = auth_client.post(subscribe_another_user_url)
        ...
        assert len(Follow.objects.all()) == 1, (
            'В БД должна остаться ровно одна запись подписки между '
        ...
        response = auth_client_another.post(subscribe_user_url)
        assert response.status_code == HTTPStatus.CREATED, (
            'Запрос зарегистрированного пользователя на создание подписки '
